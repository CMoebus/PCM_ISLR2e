...
using CSV, DataFrames,Plots, LsqFit, GLM, Statistics, Symbolics
...
# ╔═╡ a1aa0b52-e7cc-435a-b430-5230e9882b43
md"
---
...
let
	...
	kx  = round(parmsHatMyModelX[3], digits=2)            # max. growth rate k
	...
	annotate!((19.0,16,"Lx=$Lx (= Lower (left) asymptote)", 10))
	annotate!((16.9,10,"kx=$kx (= max. growth rate of standard logistic function)", 10))
	#--------------------------------------------------------------------------------
end # let

# ╔═╡ 29d37cfe-9173-4c1b-a5aa-7ca74508c6b3
md"
---
###### 3.3.3 Nonlinear Univariate OLS-Regressions using the [Generalized Logistic Function](https://en.wikipedia.org/wiki/Generalised_logistic_function)
...
let
	...
	annotate!((151.0,16,"Ly=$Ly (= Lower (left) asymptote)", 10))
	annotate!((122.9,10,"ky=$ky (= max. growth rate of standard logistic function)", 10))
	#--------------------------------------------------------------------------------
end # let
...
# ╔═╡ eeadec6a-758e-49fd-b40d-ed298552f18e
md"
---
###### 3.7.1 3D Plot Income X Education X Seniority
"

# ╔═╡ d54cdff6-b137-4b50-947e-b60cb548dba8
let
	#--------------------------------------------------------------------------------
...
The $p$-values of predictors reveal that the interaction term $Education\&Seniority$ is *not* necessary at least in the context of the other predictors.
"

# ╔═╡ 7575eb49-97d4-4bbc-8a4a-9a51cea0be08
let
	xdata = income2DataFrame.Education
	...
	zhat = predict(ols_lin_reg)
	#--------------------------------------------------------------------------------
	e    = residuals(ols_lin_reg)
	sse  = round(e'e, digits=2)
	multipleR = round(cor(zdata, zhat), digits=3)
	multipleRsQ = round(multipleR^2, digits=3)
	#--------------------------------------------------------------------------------
	plot(title="Income -> logEducation '*' logSeniority (ISLR2e, cf.Fig. 2.3)", xlims=(10,20), ylims=(25,180), zlims=(0,100), windowsize=(600*2, 400*2))
	#------------------------------------------------------------------------
