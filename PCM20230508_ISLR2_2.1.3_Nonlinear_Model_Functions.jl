...
# ╔═╡ 4f55d011-4a61-47ce-a2a4-7b4502df7733
using Plots, Symbolics, DifferentialEquations

# ╔═╡ 9e28d02c-2279-4e46-8d53-99009af90d6a
md"
==================================================================================
#### ISLR2\_2.1.3\_Nonlinear Model Functions 
##### file: PCM20230508\_ISLR2\_2.1.3\_Nonlinear\_Model\_Functions
##### code: (1.8.5/0.1\9.14) by PCM *** 2023/05/17 ***
==================================================================================
"
...
md"
---
##### 3.10 Appendix: Derivatives of Sigmoid (= Logistic) Functions

###### 3.10.1 Derivatives of the simple [*Sigmoid* (= *Logistic*) function](https://en.wikipedia.org/wiki/Logistic_function)
"
...
let
	plot(title="Derivative f'_1(x) of the Simple Logistic Function")
	...
	plot!([0,0], [0,fDeriv1(0)])
end # let

# ╔═╡ 30e2428a-e5ba-4f34-bbf5-bcbe36be0c66
let
	plot(title="Derivative f'_2(x) of the Simple Logistic Function")
	fDeriv1(x) = exp(-x)/(1+exp(-x))^2
	fDeriv2(x) = exp(x)/(1+exp(x))^2
	fDeriv3(x) = 1/(exp(x)+1) - 1/(exp(x)+1)^2
	plot!([x for x in -10:0.1:10], [fDeriv2(x) for x in -10:0.1:10])
	plot!([0,0], [0,fDeriv2(0)])
end # let

# ╔═╡ b29aca1f-2b87-4878-8b9d-f4ba57e190e8
let
	plot(title="Derivative f'_3(x) of the Simple Logistic Function")
	fDeriv1(x) = exp(-x)/(1+exp(-x))^2
	fDeriv2(x) = exp(x)/(1+exp(x))^2
	fDeriv3(x) = 1/(exp(x)+1) - 1/(exp(x)+1)^2
	plot!([x for x in -10:0.1:10], [fDeriv3(x) for x in -10:0.1:10])
	plot!([0,0], [0,fDeriv3(0)])
end # let

# ╔═╡ f2902e87-2ce1-40a5-8c0d-a0c3f1e326b2
md"
---
###### 3.10.2 Derivative of the [*Generalized Sigmoid* (= *Logistic*) function](https://en.wikipedia.org/wiki/Generalised_logistic_function)
...
let
	...
	U = 78.25
	L = 18.89
	plot(title="Gen.Log.Fun. f_g(x) = L+(U-L)/(1 + exp(-k(x-x_k_max))")
	...
	annotate!(26, 20, "Income1 Data")
end # let

# ╔═╡ 8d541667-ac3d-45d4-af22-869312c73185
let
	#--------------------------------------------------------------------------------
	plot(title="Derivative f'_g (= Logistic Diff. Equation)")
	@variables x
	...
	plot!([x_k_max, x_k_max], [0, dgl_myModel(x_k_max)])
	annotate!(27.5, 1, "Income1 Data")
	#--------------------------------------------------------------------------------
end # let

# ╔═╡ 630eee61-efdf-4694-80b5-6ee12b9e6d00
md"
---
##### 3.11 Numeric Solutions of Logistic ODEs
###### 3.11.1 [Solution](https://docs.sciml.ai/DiffEqDocs/stable/getting_started/) of ODE Logistic Differential Equation

The solution was obtained by package [DifferentialEquations.jl](DifferentialEquations.jl).
"

# ╔═╡ b7c9d2da-5883-4332-a92a-04cb054cb912
let
	fLogistic = x -> 1/(1 + exp(-x))
	fDeriv(y, p, x) = exp(-x)/(1+exp(-x))^2
	y0 = 0.0
	tspan = (-10.0, 10.0)
	problem = ODEProblem(fDeriv, y0, tspan)
	solution = solve(problem, Tsit5(), reltol = 1e-8, abstol = 1e-8)
	#----------------------------------------------------------------------
	plot(solution, linewidth = 3, title = "Solution to the Logistic ODE", xaxis = "Time (t)", yaxis = "x(t) (in μm)", label = "Solution of Logistic ODE") 
	plot!(solution.t, fLogistic, lw = 3, ls = :dash, label = "True Solution!")
end # let

# ╔═╡ 017b7454-4c72-4c31-973d-53d6bab8f94c
md"
---
###### 3.11.2 [Solution](https://docs.sciml.ai/DiffEqDocs/stable/getting_started/) of ODE General Logistic Differential Equation

The solution was obtained by package [DifferentialEquations.jl](DifferentialEquations.jl).
"

# ╔═╡ ad019b34-3824-4048-a489-9e8254bdfbcd
let
	x_k_max = 15.65
	k = 0.58
	U = 78.25
	L = 18.89
	y0 = 18.89
	fLogistic = x -> L + (U-L)/(1 + exp(-k*(x - x_k_max)))
	expExpression(x) = exp(-k*(-15.65+x))
	dgl_myModel(y, p, x) = 0.58*(59.36/(1+expExpression(x))^2)*expExpression(x)
	y0 = 18.89
	tspan = (5.0, 25.0)
	problem = ODEProblem(dgl_myModel, y0, tspan)
	solution = solve(problem, Tsit5(), reltol = 1e-8, abstol = 1e-8)
	#----------------------------------------------------------------------
	plot(solution, linewidth = 3, title = "Solution to the General Logistic ODE", xaxis = "Time (t)", yaxis = "x(t) (in μm)", label = "Solution of General Logistic ODE") 
	plot!(solution.t, fLogistic, lw = 3, ls = :dash, label = "True Solution!")
end # let

# ╔═╡ bad16089-458c-46dd-882b-434bbd2ed449
md"
---
##### 3.12 Discussion

We could demonstrate that we 
- *estimated* by the ordinary least squares (OLS) parameter estimation technique parameters of nonlinear general logistic model functions 
- *derived* symbolically the ordinary differential equation (ODE) of the general logistic model function to obtain the growth model of $y(x) = Income(Education)$
- *integrated* the ODE numerically and got a numeric solution which is identical to the original model equation
- *used* packages $Symbolics.jl$ and $Differential Equations.jl$
...
julia_version = "1.9.0"
manifest_format = "2.0"
project_hash = "8859eeacf5ac25db77219c7ab5d53eb83e7d762a"
